...
class TaroProvider extends React.Component {
  ...
  onHardwareBackPress = () => {
    ...
    if (parentState.routeName === 'tab' && parentState.index === 0) {
      const now = Date.now()
      if(now - this.closeTigger > 2000){
        this.closeTigger = now
        Taro.showToast({ title:'再按一次退出E网本地生活', icon:'none', duration: 2000})
        return true
      }
      return false
    }
